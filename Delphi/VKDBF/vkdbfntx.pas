...
type
  ...
  NTX_HEADER = packed record
    ...
    max_item: WORD;                             //2       18
    half_page: WORD;                            //2       20
    key_expr: array [0..NTX_MAX_KEY-1] of Char; //256     22
    unique: Char;                               //1      278
    reserv1: Char;                              //1      279
    desc: Char;                                 //1      280
    reserv3: Char;                              //1      281
    for_expr: array [0..NTX_MAX_KEY-1] of Char; //256    282
    order: array [0..7] of char;                //8      538
    Rest: array [0..477] of char;               //478    546
  end;                                          //1024
  ...
  NTX_ITEM = packed record
    page: DWORD;
    rec_no: DWORD;
    key: array[0..NTX_PAGE-1] of Char;
  end;
  pNTX_ITEM = ^NTX_ITEM;
  ...
  public
    ...
    constructor Create(FileName: String; key_size, BufSize: Integer); overload;
    destructor Destroy; override;
  end;
...
*)
  ...
  published
    ...
    property HiKey: String read FHiKey write FHiKey;
    property LoKey: String read FLoKey write FLoKey;
  ...
  private
    ...
    procedure SetNTXFileName(const Value: String);
    procedure SetKeyExpresion(Value: String);
    procedure SetForExpresion(Value: String);
    function CompareKeys(S1, S2: PChar; MaxLen: Cardinal): Integer;
    ...
    function SeekFirstInternal(Key: String; SoftSeek: boolean = false): boolean;
    function SeekLastInternal(Key: String; SoftSeek: boolean = false): boolean;
    procedure ChekExpression(var Value: String);
  ...
  protected
    ...
    function GetCurrentKey: String; override;
    ...
    function GetOrder: String; override;
    procedure SetOrder(Value: String); override;
  ...
  public
    ...
    function CmpKeys(ItemKey, CurrKey: pChar; KSize: Integer = 0): Integer;
    function CmpKeys1(ItemKey, CurrKey: pChar; KSize: Integer = 0): Integer;
    function CmpKeys2(ItemKey, CurrKey: pChar; KSize: Integer = 0): Integer;
    function CmpKeys3(ItemKey, CurrKey: pChar; KSize: Integer = 0): Integer;
    procedure TransKey(Key: pChar; KSize: Integer = 0; ToOem: Boolean = true); overload;
    function TransKey(Key: String): String; overload;
    ...
    function SetToRecord(Key: String; Rec: Longint): boolean; overload; override;

    //
    function Seek(Key: String; SoftSeek: boolean = false): boolean; override;
    ...
    function FindKey(Key: String; PartialKey: boolean = false; SoftSeek: boolean = false; Rec: DWORD = 0): Integer; override;
    function FindKeyFields( const KeyFields: string; const KeyValues: Variant;
                            PartialKey: boolean = false): Integer; overload; override;
    function FindKeyFields( const KeyFields: string; const KeyValues: array of const;
                            PartialKey: boolean = false): Integer; overload; override;
    function FindKeyFields( PartialKey: boolean = false): Integer; overload; override;
    //

    function SubIndex(LoKey, HiKey: String): boolean; override;
    function SubNtx(var SubNtxFile: String; LoKey, HiKey: String): boolean;
    function FillFirstBufRecords(DBFHandler: TProxyStream; FBuffer: pChar; FRecordsPerBuf: Integer; FRecordSize: Integer; FBufInd: pLongint; data_offset: Word): longint; override;
    function FillLastBufRecords(DBFHandler: TProxyStream; FBuffer: pChar; FRecordsPerBuf: Integer; FRecordSize: Integer; FBufInd: pLongint; data_offset: Word): longint; override;
    function EvaluteKeyExpr: String; override;
    function SuiteFieldList(fl: String; out m: Integer): Integer; override;
    ...
    function LastKey(out LastKey: String; out LastRec: LongInt): boolean; override;
    function NextBuffer(DBFHandler: TProxyStream; FBuffer: pChar; FRecordsPerBuf: Integer; FRecordSize: Integer; FBufInd: pLongint; data_offset: Word): Longint; override;
    function PriorBuffer(DBFHandler: TProxyStream; FBuffer: pChar; FRecordsPerBuf: Integer; FRecordSize: Integer; FBufInd: pLongint; data_offset: Word): Longint; override;
    procedure SetRangeFields(FieldList: String; FieldValues: array of const); overload; override;
    procedure SetRangeFields(FieldList: String; FieldValues: variant); overload; override;
    function InRange(Key: String): boolean; overload; override;
    ...
    procedure DeleteKey(sKey: String; nRec: Longint); override;
    procedure AddKey(sKey: String; nRec: Longint); override;
  ...
  published
    ...
    property NTXFileName: String read FNTXFileName write SetNTXFileName;
    property KeyExpresion: String read FKeyExpresion write SetKeyExpresion stored false;
    property ForExpresion: String read FForExpresion write SetForExpresion stored false;
...
function TVKNTXIndex.EvaluteKeyExpr: String;
begin
...
var
  ...
  var
    page: pNTX_BUFFER;
    item: pNTX_ITEM;
    Srckey: array[0..NTX_PAGE-1] of Char;
    Destkey: array[0..NTX_PAGE-1] of Char;
    ntxb: TVKNTXNode;
  begin
    Inc(level);
    try
      ...
      try
        item := pNTX_ITEM(pChar(page) + page.ref[0]);
  ...
  begin
    ...
    try
      ...
      try
        ...
        if page.count <> 0 then begin
          //
          item := pNTX_ITEM(pChar(page) + page.ref[page.count - 1]);
          if FKeyTranslate then begin
            Move(item.key, Srckey, FNTXOrder.FHead.key_size);
            Srckey[FNTXOrder.FHead.key_size] := #0;
            TVKDBFNTX(FIndexes.Owner).Translate(Srckey, Destkey, false);
    ...
    try
      if page.count > 0 then begin
        for i := 0 to page.count - 1 do begin
          ...
          c := CmpKeys(item.key, pChar(FCurrentKey));
          if c <= 0 then begin
            if ( FCurrentRec = item.rec_no ) and ( c = 0 ) then begin
              ...
              if ( item.page = 0 ) then begin
                if ( i <> 0 ) then begin
                  gr := grOK;
                  item := pNTX_ITEM(pChar(page) + page.ref[i - 1]);
                  SetCurrentKey;
                end;
              end else begin
...
function TVKNTXIndex.Seek(Key: String; SoftSeek: boolean = false): boolean;
var
  R: Integer;
begin
...
function TVKNTXIndex.SeekFirstInternal( Key: String; SoftSeek: boolean = false): boolean;
var
...
function TVKNTXIndex.SeekFirst( Key: String; SoftSeek: boolean = false;
...
procedure TVKNTXIndex.SetKeyExpresion(Value: String);
var
  oW: TVKDBFNTX;
begin
...
procedure TVKNTXIndex.SetForExpresion(Value: String);
var
  oW: TVKDBFNTX;
begin
  ChekExpression(Value);
...
procedure TVKNTXIndex.SetNTXFileName(const Value: String);
var
  PointPos: Integer;
begin
  FNTXFileName := Value;
...
function TVKNTXIndex.SubIndex(LoKey, HiKey: String): boolean;
var
  l, m: Integer;
    ...
    try
      if page.count > 0 then begin
        for i := 0 to page.count - 1 do begin
          ...
          c := CmpKeys(item.key, pChar(LoKey), m);
          ...
          if c <= 0 then begin //LoKey <= item.key
            ...
            c := CmpKeys(item.key, pChar(HiKey), l);
...
function TVKNTXIndex.SubNtx(var SubNtxFile: String; LoKey, HiKey: String): boolean;
var
  l, m: Integer;
  Accept: boolean;
...
function TVKNTXIndex.NextBuffer(DBFHandler: TProxyStream; FBuffer: pChar; FRecordsPerBuf: Integer; FRecordSize: Integer; FBufInd: pLongint; data_offset: Word): Longint;
var
  lResult: Longint;
  Found: boolean;
  v: WORD;
          ...
          if c <= 0 then begin //FCurrentKey <= item.key
            ...
            if Found then begin
              if NTXRange.Active then begin
                ...
                if l > 0 then begin
                  c := CmpKeys(item.key, pChar(NTXRange.HiKey), l);
              ...
              pLongint(pChar(FBufInd) + lResult * SizeOf(Longint))^ := item.rec_no;
...
function TVKNTXIndex.PriorBuffer(DBFHandler: TProxyStream; FBuffer: pChar; FRecordsPerBuf: Integer; FRecordSize: Integer; FBufInd: pLongint; data_offset: Word): Longint;
var
  lResult: Longint;
  bResult: boolean;
  Found: boolean;
  ...
  begin
    ...
    if Found then begin
      while k >= 0 do begin
        item := pNTX_ITEM(pChar(page) + page.ref[k]);
        if k < page.count then begin
          if NTXRange.Active then begin
            c := CmpKeys(item.key, pChar(NTXRange.LoKey));
            if c > 0 then begin //NTXRange.LoKey > item.key
              bResult := true;
              Exit;
            end;
          end;
          pLongint(pChar(FBufInd) + (FRecordsPerBuf - lResult - 1) * SizeOf(Longint))^ := item.rec_no;
          DBFHandler.Seek(data_offset + (item.rec_no - 1) * DWORD(FRecordSize), soFromBeginning);
...
function TVKNTXIndex.SetToRecord(Key: String; Rec: Integer): boolean;
var
  TmpKey: String;
begin
  Result := true;
...
function TVKNTXIndex.GetCurrentKey: String;
begin
  Result := FCurrentKey;
end;
...
function TVKNTXIndex.FillFirstBufRecords(DBFHandler: TProxyStream; FBuffer: pChar;
...
begin
  ...
  if not NTXRange.Active then begin
    ...
    if FSeekOk then begin
      ...
      c := CmpKeys2(pChar(NTXRange.LoKey), pChar(FSeekKey), l);
      if c >= 0 then begin
        l := Length(NTXRange.HiKey);
        c := CmpKeys2(pChar(NTXRange.HiKey), pChar(FSeekKey), l);
        if (l > 0) and (c <= 0) then begin
          ...
          Result := 1 + NextBuffer(DBFHandler, FBuffer + FRecordSize, FRecordsPerBuf - 1, FRecordSize, pLongint(pChar(FBufInd) + SizeOf(Longint)), data_offset);
        end else
          Result := 0;
      end else
        Result := 0;
...
function TVKNTXIndex.FillLastBufRecords(DBFHandler: TProxyStream; FBuffer: pChar;
    ...
    if FSeekOk then begin
      ...
      if c >= 0 then begin
        ...
        if (l > 0) and (c <= 0) then begin
          FCurrentKey := TransKey(FSeekKey);
          FCurrentRec := FSeekRecord;
          pLongint(pChar(FBufInd) + (FRecordsPerBuf - 1) * SizeOf(Longint))^ := FCurrentRec;
...
function TVKNTXIndex.CompareKeys(S1, S2: PChar; MaxLen: Cardinal): Integer;
var
  i: Integer;
  T1: array [0..NTX_PAGE] of Char;
  T2: array [0..NTX_PAGE] of Char;
begin
  //S1 - CurrentKey
  //S2 - Item Key
  if Assigned(OnCompareKeys) then begin
    ...
    if CollationType <> cltNone then begin
      ...
      TVKDBFNTX(FIndexes.Owner).Translate(S1, pChar(@T1[0]), True);
      TVKDBFNTX(FIndexes.Owner).Translate(S2, pChar(@T2[0]), True);
      //end TODO
...
begin
  if FNTXOrder.FHead.next_page <> 0 then begin
    ...
    FNTXOrder.FHead.next_page := pNTX_ITEM(pChar(page) + page.ref[page.count]).page;
    pNTX_ITEM(pChar(page) + page.ref[page.count]).page := 0;
    ...
    pNTX_ITEM(pChar(page) + page.ref[0]).page := 0;
...
var
  ...
  var
    ...
    begin
      ...
      Move(ntxItem.key, pNTX_ITEM(pChar(page) + page.ref[i]).key, FNTXOrder.FHead.key_size);
      pNTX_ITEM(pChar(page) + page.ref[i]).rec_no := ntxItem.rec_no;
      pNTX_ITEM(pChar(page) + page.ref[i]).page := ntxItem.page;
      if ( ntxItem.page <> 0 ) then begin
        pNTX_ITEM(pChar(page) + page.ref[i + 1]).page := NewPageOff;
        NewPageOff := 0;
      end;
    end;
  ...
  begin
    ...
    try
      ...
      if not AddItemAtEnd then begin
        if ( i > 0 ) then begin
          beg := 0;
          item := pNTX_ITEM(pChar(page) + page.ref[beg]);
          c := CmpKeys1(item.key, pChar(@ntxItem^.key[0]));
          ...
          if ( c > 0 ) then begin
            repeat
              Mid := ( i + beg ) shr 1;
              item := pNTX_ITEM(pChar(page) + page.ref[Mid]);
              c := CmpKeys1(item.key, pChar(@ntxItem^.key[0]));
      ...
      if not ItemHasBeenAdded then begin
        ...
        if AddItemAtEnd and ( page.count = FNTXOrder.FHead.max_item ) then begin
          if ( ParentNode <> nil )
            ...
            SiblingItem := pNTX_ITEM(pChar(ParentNode.pNode) + ParentNode.PNode.ref[Pred(ParentItemNo)]);
            ...
            try
              ...
              for k := SiblingNodePage.count to Pred(FNTXOrder.FHead.max_item) do begin
                i1 := pNTX_ITEM(pChar(SiblingNode.pNode) + SiblingNode.PNode.ref[k]);
                i2 := pNTX_ITEM(pChar(Ind.pNode) + Ind.PNode.ref[l]);
                i1.rec_no := SiblingItem.rec_no;
                Move(SiblingItem.key, i1.key, FNTXOrder.FHead.key_size);
                i1 := pNTX_ITEM(pChar(SiblingNode.pNode) + SiblingNode.PNode.ref[Succ(k)]);
                i1.page := i2.page;
                i2.page := 0;
        ...
        if (page.count = FNTXOrder.FHead.max_item) then begin
          ...
          try
            ...
            if page.count >= NewPage.count then begin
              page.count := page.count - 1;
              Move(pNTX_ITEM(pChar(page) + page.ref[page.count]).key, ntxItem.key, FNTXOrder.FHead.key_size);
              ntxItem.rec_no := pNTX_ITEM(pChar(page) + page.ref[page.count]).rec_no;
            end else begin
              Move(pNTX_ITEM(pChar(NewPage) + NewPage.ref[0]).key, ntxItem.key, FNTXOrder.FHead.key_size);
              ntxItem.rec_no := pNTX_ITEM(pChar(NewPage) + NewPage.ref[0]).rec_no;
            ...
            RootUseCount := Ind.NTXBuffer.UseCount;

            Ind.NTXBuffer.UseCount := RootUseCount shr 1;
            Ind.Changed := true;

            Ind1.NTXBuffer.UseCount := RootUseCount shr 1;
            Ind1.Changed := true;
          finally
...
begin
  ...
  if not ItemHasBeenAdded then begin
    ...
    try
      NewPage.count := 1;
      Move(ntxItem.key, pNTX_ITEM(pChar(NewPage) + NewPage.ref[0]).key, FNTXOrder.FHead.key_size);
      pNTX_ITEM(pChar(NewPage) + NewPage.ref[0]).rec_no := ntxItem.rec_no;
      pNTX_ITEM(pChar(NewPage) + NewPage.ref[0]).page := ntxItem.page;
      pNTX_ITEM(pChar(NewPage) + NewPage.ref[1]).page := NewPageOff;
      FNTXOrder.FHead.root := _NewPageOff;

      Ind.NTXBuffer.UseCount := RootUseCount;
      Ind.Changed := True;
    finally
      FNTXBuffers.FreeNTXBuffer(Ind.FNTXBuffer);
    end;
...
procedure TVKNTXIndex.AddKey(sKey: String; nRec: Integer);
var
  item: NTX_ITEM;
  AddOk: boolean;
begin
  ...
  if AddOk then begin
    item.page := 0;
    item.rec_no := nRec;
    Move(pChar(sKey)^, item.key, FNTXOrder.FHead.key_size);
    TransKey(item.key);
    AddItem(@item);
  end;
end;

procedure TVKNTXIndex.DeleteKey(sKey: String; nRec: Integer);
var
  TempItem: NTX_ITEM;
  LastItem: NTX_ITEM;
  FLastKey: String;
  ...
  procedure AddInEndItem(page_off: DWORD; itemKey: pChar; itemRec: DWORD);
  var
    page: pNTX_BUFFER;
    NewPage: pNTX_BUFFER;
    item: pNTX_ITEM;
  ...
  begin
    ...
    try
      ...
      if ( item.page <> 0 ) then
        ...
        if page.count < FNTXOrder.FHead.max_item then begin
          Move(itemKey^, pNTX_ITEM(pChar(page) + page.ref[page.count]).key, FNTXOrder.FHead.key_size);
          pNTX_ITEM(pChar(page) + page.ref[page.count]).rec_no := itemRec;
          pNTX_ITEM(pChar(page) + page.ref[page.count]).page := 0;
          page.count := page.count + 1;
          pNTX_ITEM(pChar(page) + page.ref[page.count]).page := 0;
          ...
          try
            ...
            pNTX_ITEM(pChar(page) + page.ref[FNTXOrder.FHead.half_page]).page := NewPageOff;
            ...
            Move(itemKey^, pNTX_ITEM(pChar(NewPage) + NewPage.ref[NewPage.count]).key, FNTXOrder.FHead.key_size);
            pNTX_ITEM(pChar(NewPage) + NewPage.ref[NewPage.count]).rec_no := itemRec;
            pNTX_ITEM(pChar(NewPage) + NewPage.ref[NewPage.count]).page := 0;
            NewPage.count := NewPage.count + 1;
  ...
  begin
    ...
    page.count := 0;
    pNTX_ITEM(pChar(page) + page.ref[0]).page := FNTXOrder.FHead.next_page;
  ...
  begin
    ...
    try
      ...
      if ( page.count = 0 ) and ( item.page = 0 ) then begin
        DeletePage(page_off);
        pNTX_ITEM(pChar(PrePage) + NTX_BUFFER(PrePage^).ref[PreItemRef])^.page := 0;
  ...
  var
    ...
    begin
      Result := false;
      if page.count = 0 then begin
        item1 := pNTX_ITEM(pChar(page) + page.ref[0]);
        if ( item1.page = 0 ) then begin
          if LastPage <> nil then begin
            pNTX_ITEM(pChar(LastPage) + NTX_BUFFER(LastPage^).ref[LastItemRef])^.page := 0;
            DeletePage(page_off);
          ...
          if c <= 0 then begin //sKey <= item.key
            ...
            if ( DWORD(nRec) = item.rec_no ) and ( c = 0 ) then begin
              if ( item.page = 0 ) then begin
                ...
                Move(pChar(FLastKey)^, pNTX_ITEM(pChar(page) + page.ref[i]).key, FNTXOrder.FHead.key_size);
                pNTX_ITEM(pChar(page) + page.ref[i]).rec_no := FLastRec;
      ...
      if Parent <> nil then begin
        if CurrPage.PNode.count < FNTXOrder.FHead.half_page then begin
          ...
          if ParentItemRef > 0 then begin
            LItem := pNTX_ITEM( pChar(Parent.PNode) + Parent.PNode.ref[ParentItemRef - 1]);
            if LItem.page <> 0 then begin
              ...
              if LeftPage.count > FNTXOrder.FHead.half_page then begin
                ...
                SLItem := pNTX_ITEM( pChar(LeftPage) + LeftPage.ref[LeftPage.count]);
                ...
                CItem := pNTX_ITEM( pChar(CurrPage.PNode) + CurrPage.PNode.ref[Shift - 1]);
                ...
                while Shift > 0 do begin
                  ...
                  SLItem := pNTX_ITEM( pChar(LeftPage) + LeftPage.ref[LeftPage.count + Shift]);
                ...
                LItem.rec_no := SLItem.rec_no;
                  ...
                  CItem := pNTX_ITEM( pChar(Parent.PNode) + Parent.PNode.ref[ParentItemRef]);
                  Item := pNTX_ITEM( pChar(LeftPage) + LeftPage.ref[LeftPage.count]);
                  ...
                  for j := 0 to CurrPage.PNode.count do begin
                    CItem := pNTX_ITEM( pChar(CurrPage.PNode) + CurrPage.PNode.ref[j]);
                    Item := pNTX_ITEM( pChar(LeftPage) + LeftPage.ref[LeftPage.count]);
          ...
          if TryRight then begin
            if ParentItemRef < Parent.PNode.count then begin
              RItem := pNTX_ITEM( pChar(Parent.PNode) + Parent.PNode.ref[ParentItemRef + 1]);
              if RItem.page <> 0 then begin
                ...
                if RightPage.count > FNTXOrder.FHead.half_page then begin
                  ...
                  CItem := pNTX_ITEM( pChar(Parent.PNode) + Parent.PNode.ref[ParentItemRef]);
                  Item := pNTX_ITEM( pChar(CurrPage.PNode) + CurrPage.PNode.ref[CurrPage.PNode.count]);
                  Move(CItem.key, Item.key, FNTXOrder.FHead.key_size);
                  Item.rec_no := CItem.rec_no;

                  Inc(CurrPage.PNode.count);

                  Item := pNTX_ITEM( pChar(RightSibling.PNode) + RightSibling.PNode.ref[Shift - 1]);
                  Move(Item.key, CItem.key, FNTXOrder.FHead.key_size);
                  CItem.rec_no := Item.rec_no;
                  LstPage := Item.page;

                  for j := 0 to Shift - 2 do begin
                    SRItem := pNTX_ITEM( pChar(RightSibling.PNode) + RightSibling.PNode.ref[j]);
                    Item := pNTX_ITEM( pChar(CurrPage.PNode) + CurrPage.PNode.ref[CurrPage.PNode.count]);
                  ...
                  CItem := pNTX_ITEM( pChar(RightSibling.PNode) + RightSibling.PNode.ref[0]);
                  CItem.page := FNTXOrder.FHead.next_page;
...
begin
  ...
  if FDeleteKeyStyle = dksClipper then begin
    ...
    Move(pChar(sKey)^, TempItem.key, FNTXOrder.FHead.key_size);
...
function TVKNTXIndex.LastKey(out LastKey: String; out LastRec: Integer): boolean;
var
  level: Integer;
...
function TVKNTXIndex.GetOrder: String;
var
  i: Integer;
  p: pChar;
begin
  if IsOpen then begin
    ...
    p := pChar(@FNTXOrder.FHead.order[0]);
...
procedure TVKNTXIndex.SetOrder(Value: String);
var
  i, j: Integer;
begin
  if IsOpen then begin
...
procedure TVKNTXIndex.ChekExpression(var Value: String);
var
  i, j: Integer;
begin
  j := Length(Value);
...
function TVKNTXIndex.CmpKeys1(ItemKey, CurrKey: pChar; KSize: Integer): Integer;
var
  Srckey: array[0..NTX_PAGE-1] of Char;
  Destkey1, Destkey2: array[0..NTX_PAGE-1] of Char;
begin
...
function TVKNTXIndex.CmpKeys2(ItemKey, CurrKey: pChar; KSize: Integer): Integer;
var
  Srckey: array[0..NTX_PAGE-1] of Char;
  Destkey1, Destkey2: array[0..NTX_PAGE-1] of Char;
begin
...
function TVKNTXIndex.CmpKeys(ItemKey, CurrKey: pChar; KSize: Integer = 0): Integer;
var
  Srckey: array[0..NTX_PAGE-1] of Char;
  Destkey: array[0..NTX_PAGE-1] of Char;
begin
...
function TVKNTXIndex.CmpKeys3(ItemKey, CurrKey: pChar; KSize: Integer): Integer;
begin
  if KSize = 0 then KSize := FNTXOrder.FHead.key_size;
  Result := CompareKeys(CurrKey, ItemKey, KSize);
  if Desc then Result := - Result;
...
procedure TVKNTXIndex.TransKey(Key: pChar; KSize: Integer = 0; ToOem: Boolean = true);
var
  Srckey: array[0..NTX_PAGE-1] of Char;
begin
  if KSize = 0 then KSize := FNTXOrder.FHead.key_size;
...
begin
  if TVKDBFNTX(FIndexes.Owner).OEM and ( CollationType <> cltNone ) then begin
    TVKDBFNTX(FIndexes.Owner).TranslateBuff(pChar(@SItem.key[0]), pChar(@SItem.key[0]), True, FNTXOrder.FHead.key_size);
  end;
end;
...
var
  ...
  procedure AddKeyInSorter(Key: String; Rec: DWORD);
  var
    AddOk: boolean;
  begin
    AddOk := true;
    ...
    if AddOk then begin
      ...
      if TVKDBFNTX(FIndexes.Owner).OEM and ( CollationType <> cltNone ) then
        TVKDBFNTX(FIndexes.Owner).TranslateBuff(pChar(Key), pChar(@sitem.key[0]), True, FNTXOrder.FHead.key_size)
      else
        Move(pChar(Key)^, sitem.key, FNTXOrder.FHead.key_size);
    ...
    if not FReindex then begin
      ...
      if not FNTXBag.NTXHandler.IsOpen then begin
        ...
        if Assigned(OnCreateIndex) then begin
          ...
          System.Move(pChar(IndAttr.key_expr)^, FNTXOrder.FHead.key_expr, Length(IndAttr.key_expr));
          FNTXOrder.FHead.key_expr[Length(IndAttr.key_expr)] := #0;
          System.Move(pChar(IndAttr.for_expr)^, FNTXOrder.FHead.for_expr, Length(IndAttr.for_expr));
          ...
          System.Move(pChar(FKeyExpresion)^, FNTXOrder.FHead.key_expr, Length(FKeyExpresion));
          FNTXOrder.FHead.key_expr[Length(FKeyExpresion)] := #0;
          System.Move(pChar(FForExpresion)^, FNTXOrder.FHead.for_expr, Length(FForExpresion));
                ...
                if not Sorter.FirstSortItem then begin
                  repeat
                    ...
                    if oB.OEM and ( CollationType <> cltNone ) then
                      ...
                      TVKDBFNTX(FIndexes.Owner).TranslateBuff(pChar(@SortItem.key[0]), pChar(@ntxitem.key[0]), True, FNTXOrder.FHead.key_size);
                    ntxitem.key[FNTXOrder.FHead.key_size] := #0;

                    SetString(Key, pChar(@ntxitem.key[0]), FNTXOrder.FHead.key_size);
                    TransKey(pChar(Key), FNTXOrder.FHead.key_size, False);
                    AddOk := true;
                    if Unique then
...
var
  ...
  procedure LoadBlock(BlockFile: String; pBlock: pChar);
  var
    h: Integer;
  begin
  ...
  begin
    ...
    if Assigned(OnCreateIndex) then begin
      ...
      System.Move(pChar(IndAttr.key_expr)^, FNtxHead.key_expr, Length(IndAttr.key_expr));
      FNtxHead.key_expr[Length(IndAttr.key_expr)] := #0;
      System.Move(pChar(IndAttr.for_expr)^, FNtxHead.for_expr, Length(IndAttr.for_expr));
      FNtxHead.for_expr[Length(IndAttr.for_expr)] := #0;
    end else begin
      FNtxHead.key_size := Length(FKeyParser.Key);
      FNtxHead.key_dec := FKeyParser.Prec;
      System.Move(pChar(FKeyExpresion)^, FNtxHead.key_expr, Length(FKeyExpresion));
      FNtxHead.key_expr[Length(FKeyExpresion)] := #0;
      System.Move(pChar(FForExpresion)^, FNtxHead.for_expr, Length(FForExpresion));
  ...
  procedure InitBlock(Block: pChar);
  var
    page: pBLOCK_BUFFER;
  ...
  procedure AddKeyInBlock(Key: String; Rec: DWORD);
  var
    AddOk: boolean;
    i, j, beg, Mid: Integer;
    page: pBLOCK_BUFFER;
    ...
    begin
      ...
      page.count := page.count + 1;
      Move(pChar(Key)^, pBLOCK_ITEM(pChar(page) + page.ref[i]).key, FNTXOrder.FHead.key_size);
      pBLOCK_ITEM(pChar(page) + page.ref[i]).rec_no := Rec;
    end;
  ...
  begin
    ...
    if AddOk then begin
      ...
      if ( i > 0 ) then begin
        beg := 0;
        item := pBLOCK_ITEM(pChar(page) + page.ref[beg]);
        c := CmpKeys1(item.key, pChar(Key));
        ...
        if ( c > 0 ) then begin
          repeat
            Mid := (i+beg) div 2;
            item := pBLOCK_ITEM(pChar(page) + page.ref[Mid]);
            c := CmpKeys1(item.key, pChar(Key));
            if ( c = 0 ) and Unique then Exit;
...
function TVKNTXIndex.SuiteFieldList(fl: String; out m: Integer): Integer;
begin
  if Temp then begin
    m := 0;
    Result := 0
...
function TVKNTXIndex.SeekFields(const KeyFields: string;
...
function TVKNTXIndex.SeekLastInternal(Key: String;
    ...
    try
      if page.count > 0 then begin
        for i := 0 to page.count - 1 do begin
          ...
          c := CmpKeys(item.key, pChar(Key), Length(Key));
...
procedure TVKNTXIndex.SetRangeFields(FieldList: String;
...
function TVKNTXIndex.InRange(Key: String): boolean;
begin
...
function TVKNTXIndex.SeekFirstRecord(Key: String;
...
function TVKNTXIndex.FindKey( Key: String; PartialKey: boolean = false;
    ...
    try
      if page.count > 0 then begin
        for i := 0 to page.count - 1 do begin
          ...
          c := CmpKeys(item.key, pChar(Key), m);
...
function TVKNTXIndex.FindKeyFields(const KeyFields: string;
  const KeyValues: Variant; PartialKey: boolean = false): Integer;
var
  m, l: Integer;
  Key: String;
  KeyFields_: string;
...
begin
  ...
  if m > 0 then begin
    ...
    Result := FindKey(Key, PartialKeyInternal);
  end;
end;

function TVKNTXIndex.FindKeyFields(const KeyFields: string;
  const KeyValues: array of const; PartialKey: boolean = false): Integer;
var
  FieldVal: Variant;
begin
  ArrayOfConstant2Variant(KeyValues, FieldVal);
  Result := FindKeyFields(KeyFields, FieldVal, PartialKey);
end;

function TVKNTXIndex.FindKeyFields(PartialKey: boolean = false): Integer;
var
  Key: String;
begin
  if PartialKey then FKeyParser.PartualKeyValue := True;
  ...
  Result := FindKey(Key, PartialKey);
end;

function TVKNTXIndex.TransKey(Key: String): String;
begin
  Result := Key;
  TransKey(pChar(Result), Length(Result), false);
end;
...
var
  ...
  begin
    ...
    if AddOk then begin
      ...
      sitem.Hash := FKeyParser.Hash;
      if TVKDBFNTX(FIndexes.Owner).OEM and ( CollationType <> cltNone ) then
        TVKDBFNTX(FIndexes.Owner).TranslateBuff(pChar(Key), pChar(@sitem.key[0]), True, FNtxHead.key_size)
      else
        Move(pChar(Key)^, sitem.key, FNtxHead.key_size);
      Sorter.AddItem(sitem);
    end;
  end;
          ...
          if not Sorter.FirstSortItem then begin
            repeat
              ...
              if oB.OEM and ( CollationType <> cltNone ) then
                ...
                TVKDBFNTX(FIndexes.Owner).TranslateBuff(pChar(@SortItem.key[0]), pChar(@ntxitem.key[0]), True, FNtxHead.key_size);
              ntxitem.key[FNTXOrder.FHead.key_size] := #0;
  ...
  var
    ...
    begin
      ...
      if    ( ParentNode <> nil )
        ...
        SiblingItem := pNTX_ITEM(pChar(ParentNode.PNode) + ParentNode.PNode.ref[Succ(PreviousItemNo)]);
        SiblingNode := FNTXBuffers.GetNTXBuffer(FNTXBag.NTXHandler, SiblingItem.page, SiblingNodePage);
        try
          //
          if SiblingNodePage.count < FNTXOrder.FHead.max_item then begin

            Item1 := pNTX_ITEM(pChar(CurrentNode.PNode) + CurrentNode.PNode.ref[Pred(CurrentNode.PNode.count)]);
            ...
            Item := pNTX_ITEM(pChar(SiblingNodePage) + SiblingNodePage.ref[0]);
            ...
            Item := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[i]);
      ...
      if    ( not IsAddInSibling )
        ...
        SiblingItem := pNTX_ITEM(pChar(ParentNode.pNode) + ParentNode.PNode.ref[Pred(PreviousItemNo)]);
        SiblingNode := FNTXBuffers.GetNTXBuffer(FNTXBag.NTXHandler, SiblingItem.page, SiblingNodePage);
        try
          //
          if SiblingNodePage.count < FNTXOrder.FHead.max_item then begin
            if i > 0 then begin
              ...
              Item1 := pNTX_ITEM(pChar(CurrentNode.PNode) + CurrentNode.PNode.ref[0]);
              ...
              Item := pNTX_ITEM(pChar(SiblingNodePage) + SiblingNodePage.ref[SiblingNodePage.count]);
      ...
      if    ( not IsAddInSibling )
        ...
        try
          ...
          if SiblingNodePage.count < FNTXOrder.FHead.max_item then begin
              ...
              Item := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[CurrentNodePage.count]);
    ...
    try
      ...
      if CurrentNodePage.count > 0 then begin
        ...
        while i < CurrentNodePage.count do begin
          ...
          c := CmpKeys1(pChar(@SortItem.key[0]), Item.key, FNTXOrder.FHead.key_size);
          ...
          if c > 0 then begin //Item.key > SortItem
            ...
            if c > 0 then begin
              ...
              if IsLeaf then begin
                ...
                TransKey(pChar(@sitem.key[0]), FNTXOrder.FHead.key_size, false);
                try
                  FGetHashFromSortItem := False;
                ...
                TransKey(pChar(@sitem.key[0]), FNTXOrder.FHead.key_size, true);
          ...
          while True do begin
            if IsRoot then
              ...
              if PreviousItem <> nil then
                c := CmpKeys1(pChar(@SortItem.key[0]), PreviousItem.key, FNTXOrder.FHead.key_size)
              else
                Break;
            end;
            if ( CurrentNodePage.count < FNTXOrder.FHead.max_item ) then begin
...
var
  ...
  procedure AddKeyInSorter(Key: pChar; Rec: DWORD);
  var
    AddOk: boolean;
  begin
    AddOk := true;
  ...
  begin
    ...
    CurrentNode := FNTXBuffers.GetNTXBuffer(FNTXBag.NTXHandler, CurrentPageOffset, CurrentNodePage);
    try
      LItem := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[CurrentNodePage.count]);
  ...
  procedure AddKeyInBTreeAtEnd(Key: String; Rec: DWORD);
  var
    AddOk: boolean;
  begin
    AddOk := true;
    ...
    if AddOk then begin
      ntxitem.page := 0;
      ntxitem.rec_no := Rec;
      Move(pChar(Key)^, ntxitem.key, FNTXOrder.FHead.key_size);
      ntxitem.key[FNTXOrder.FHead.key_size] := #0;
      TransKey(pChar(@ntxitem.key[0]));
      if Unique then
        AddOk := AddOk and (not SeekFirstInternal(Key));
      if AddOk then
        AddItem(@ntxitem, True);
...
var
  ...
  var
    Ipoint, Ip: pNTX_ITEM;
  begin
    Ipoint := pNTX_ITEM(pChar(NtxPage) + NtxPage.ref[point]);
    Ip := pNTX_ITEM(pChar(NtxPage) + NtxPage.ref[p]);
...
var
  ...
  begin
    ...
    LastItem := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[CurrentNodePage.count]);
    //Result := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[Pred(CurrentNodePage.count)]);
    Result := -1;
    ...
    MoveItem := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[Pred(CurrentNodePage.count)]);
    repeat
      m := ( b + e ) shr 1;
      Item := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[m]);
    ...
    for i := Pred(CurrentNodePage.count) downto e do begin
      Item := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[i]);
      MoveItem := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[Pred(i)]);
      //Result := MoveItem;
      if i <> l then begin
  ...
  begin
    ...
    while True do begin
      if i <= e then begin
        if (i + 1) <= e then begin
          Item1 := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[i]);
          Item2 := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[Succ(i)]);
      ...
      while True do begin
        if i < q_Count then begin
          if (i + 1) < q_Count then begin
            Item1 := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[q[i]]);
            Item2 := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[q[Succ(i)]]);
            c := CmpKeys1(Item1.key, Item2.key, FNTXOrder.FHead.key_size);
            if c = 0 then begin
              if Item2.rec_no < Item1.rec_no then
                c := -1
    ...
    MinItem := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[q[0]]);
    MinItemNo := q[0];
      ...
      if CurrentNodePage.count > 0 then begin
        ...
        if ( LastItem.rec_no = DWord(-1) ) or ( LastItem.rec_no < CurrentNodePage.count ) then begin
          ...
          if i >= 0 then begin
            ...
            //MinItem := pNTX_ITEM(pChar(CurrentNodePage) + CurrentNodePage.ref[Pred(CurrentNodePage.count)]);
  ...
  begin
    ...
    try
      ...
      for i := 0 to Pred(CurrentNodePage.count) do begin
        ...
        //
        if j >= 0 then begin
          //MinItem := pNTX_ITEM(pChar(HeapRootNodePage) + HeapRootNodePage.ref[Pred(HeapRootNodePage.count)]);
      ...
      if ( HeapRootNode.NodeOffset = CurrentNode.NodeOffset ) then begin
        ...
        LastItemInHeap := pNTX_ITEM(pChar(HeapRootNodePage) + HeapRootNodePage.ref[HeapRootNodePage.count]);
      end;
      //
...
var
  ...
  begin
    sign := False;
    if Char(Item.key[0]) in [',', '#', '$', '%', '&', '''', '(', ')', '*', '+'] then begin
      sign := True;
    ...
    SetString(S, pChar(@key[0]), FNTXOrder.FHead.key_size);
  end;
...
begin
  if FGetHashFromSortItem then
    ...
    case FKeyParser.ValueType of
      ...
      varDate:
        begin
          SetString(S, pChar(@key[0]), 4);
          y := StrToInt(S);
          SetString(S, pChar(@key[4]), 2);
          m := StrToInt(S);
          SetString(S, pChar(@key[6]), 2);
          d := StrToInt(S);
...
begin
  ...
  if l > 0 then begin
    c := NTX.CompareKeys(pChar(HiKey), pChar(S), l);
    Result := (c >= 0); //HiKey >= S
    if Result then begin
      l := min(Length(LoKey), NTX.FNTXOrder.FHead.key_size);
      if l > 0 then begin
        c := NTX.CompareKeys(pChar(LoKey), pChar(S), l);
        Result := (c <= 0); //LoKey <= S
      end;
    end;
  end else
...
begin
  result := pNTX_BUFFER( pChar( FPages ) + ( NTX_PAGE * ndx) );
end;
    ...
    if ntxb.Changed then begin
      ...
      for j := 0 to Pred(PagesPerBuffer) do begin
        if ntxb.NTXNode[j].Changed then begin
          ...
          WriteBufferStart := pNTX_BUFFER(pChar(ntxb.Pages) + (NTX_PAGE * j)); //ntxb.NTXNode[j].PNode;
          k := j;
          Break;
        end;
      end;
...
begin
  if ntxb.Changed then begin
    ...
    for j := 0 to Pred(PagesPerBuffer) do begin
      if ntxb.NTXNode[j].Changed then begin
        WriteOffsetStart := ntxb.NTXNode[j].NodeOffset;
        WriteBufferStart := pNTX_BUFFER(pChar(ntxb.Pages) + (NTX_PAGE * j));
        k := j;
        Break;
      end;
    end;
...
begin
  ...
  pNTX_ITEM(pChar(@levels[lev]) + levels[lev].ref[0]).page := 0;
  max_lev := lev;
end;
  ...
  begin
    ...
    if Parent <> nil then begin
      if CurrentPage.PNode.count < SHead.half_page then begin
        LItem := pNTX_ITEM( pChar(Parent.PNode) + Parent.PNode.ref[Parent.PNode.count - 1]);
        GetPage(LItem.page, LeftPage);

        SLItem := pNTX_ITEM( pChar(LeftPage.PNode) + LeftPage.PNode.ref[LeftPage.PNode.count]);
        ...
        CItem := pNTX_ITEM( pChar(CurrentPage.PNode) + CurrentPage.PNode.ref[Shift - 1]);
        ...
        while Shift > 0 do begin
          ...
          SLItem := pNTX_ITEM( pChar(LeftPage.PNode) + LeftPage.PNode.ref[LeftPage.PNode.count + Shift]);
    ...
    Item := pNTX_ITEM( pChar(CurrentPage.PNode) + CurrentPage.PNode.ref[CurrentPage.PNode.count]);
    if Item.page <> 0 then
...
begin
  ...
  if (cur_lev <= max_lev) then begin
    ...
    i := pNTX_ITEM(pChar(page) + page.ref[page.count]);
    i.page := SubOffSet;
    if Handler = nil then
  ...
  if page.count = SHead.max_item then begin
    ...
    if Crypt then begin
      ...
      if wbAmount > 1 then begin
        ...
        Move(CryptPage, pChar(pWriteBuff + NTX_PAGE * wbCount)^, NTX_PAGE);
        Inc(wbCount);
      end else begin
        if Handler = nil then
          SysUtils.FileWrite(FHndl, CryptPage, SizeOf(NTX_BUFFER))
      ...
      if wbAmount > 1 then begin
        ...
        Move(page^, pChar(pWriteBuff + NTX_PAGE * wbCount)^, NTX_PAGE);
        Inc(wbCount);
      end else begin
        if Handler = nil then
          SysUtils.FileWrite(FHndl, page^, SizeOf(NTX_BUFFER))
    ...
    Move(item^, (pChar(page) + page.ref[page.count])^, SHead.item_size);
    page.count := page.count + 1;
  end;
  Dec(cur_lev);
end;
...
constructor TVKNTXBlockIterator.Create(FileName: String; key_size, BufSize: Integer);
begin
  inherited Create;
  FFileName := FileName;
  Fkey_size := key_size;
...
begin
  ...
  if i >= p.count then Eof := true else begin
    item.page := 0;
    BlockItem := pBLOCK_ITEM(pChar(p) + p.ref[i]);
...
constructor TVKNTXIterator.Create(FileName: String);
begin
  inherited Create;
  FFileName := FileName;
end;
...
begin
  ...
  repeat
    page := pNTX_BUFFER(@levels^[cur_lev]);
    i := pNTX_ITEM(pChar(page) + page.ref[indexes[cur_lev]]);
...
var
  ...
  function EvaluteKeyExpr: String;
  begin
...
begin
  ...
  if Assigned(OnCreateIndex) then begin
    ...
    System.Move(pChar(IndAttr.key_expr)^, FHead.key_expr, Length(IndAttr.key_expr));
    FHead.key_expr[Length(IndAttr.key_expr)] := #0;
    System.Move(pChar(IndAttr.for_expr)^, FHead.for_expr, Length(IndAttr.for_expr));
    ...
    System.Move(pChar(KeyExpresion)^, FHead.key_expr, Length(KeyExpresion));
    FHead.key_expr[Length(KeyExpresion)] := #0;
    System.Move(pChar(ForExpresion)^, FHead.for_expr, Length(ForExpresion));
  ...
  System.Move(pChar(Name)^, FHead.order, Length(Name));
