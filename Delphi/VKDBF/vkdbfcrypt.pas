...
type
  ...
  public
    ...
    procedure CodeFile(const Source, Dest: String; Encode: Boolean);
  ...
  { XOR by Gamma method }
  function XORActivate(Password: String): LongWord;
  ...
  function GostActivate(Password: String): LongWord;
...
procedure TGostCrypt.CodeFile(const Source, Dest: String; Encode: Boolean);
var
  S,D: TFileStream;
begin
  S := nil;
...
begin
  for i := 0 to Size - 1 do
    Byte(pChar(p1)[i]) := Byte(pChar(p1)[i]) xor Byte(pChar(p2)[i]);
end;
