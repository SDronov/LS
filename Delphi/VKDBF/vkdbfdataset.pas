...
uses
  ...
  {$IFDEF VER150} Variants, FmtBcd, {$ENDIF}
...
type
  ...
  EVkDbfFileOpenError = class(Exception); 
...
////////////////////////////////////////////////////////////////////////////////
  ...
  private
    ...
    function Get_field_name: pChar;
    function Get_plendth: plen_info;
    function GetFeildName: String;
    procedure SetFeildName(const Value: String);
  ...
  public
    ...
    property field_name: pChar read Get_field_name;
    property FeildName: String read GetFeildName write SetFeildName;
    property field_type: char read Get_field_type write Set_field_type;
  ...
  public
    ...
    function SetNativeAutoInc(const FieldName: String; Value: DWORD): boolean; overload;
    function SetNativeAutoInc(const FieldNum: Integer; Value: DWORD): boolean; overload;

    function GetCurrentNativeAutoInc(const FieldName: String): DWORD; overload;
    function GetCurrentNativeAutoInc(const FieldNum: Integer): DWORD; overload;

    function GetNextNativeAutoInc(const FieldName: String): DWORD; overload;
...
uses
  Dialogs, DBcommon, ActiveX, StrUtils;
...
var
  ...
  var
    i: Integer;
    cField: FIELD_REC;
    q: array[0..Pred(FIELD_RECV_SIZE)] of Char;
  begin
    ...
    begin
      ...
      cField.NativeNextAutoInc := 1;
      cField.NativeStepAutoInc := 1;
...
begin
  ...
  begin
    ...
    if (Indexes <> nil) and (DBFIndexDefs.Count > 0) then begin
              ...
              begin
                if qq.field_type <> 'E' then begin
                  ...
                  if FTrimCType then begin
                    dInt := Pred(qq.FLen);
                    while pChar(Buffer)[dInt] = ' ' do begin
                      pChar(Buffer)[dInt] := #0;
                      Dec(dInt);
...
begin
  ...
  if not DBFHandler.IsOpen then begin
      i := GetLastError;
      raise EVkDbfFileOpenError.CreateFmt('TVKSmartDBF.InternalOpen: Open error "%s". Код ошибки %d:%s.',
                    [DBFFileName, i, SysErrorMessage(I)]);
  end else begin
    DBFHandler.Seek(0, 0);
...
begin
  ...
  if not l then begin
    ...
    case FLockProtocol of
      lpNone            : Result := True;
      lpDB4Lock         : Result := DBFHandler.UnLock(DB4_LockMin,  DB4_LockRng);
      lpClipperLock     : Result := DBFHandler.UnLock(CLIPPER_LockMin + 1 , CLIPPER_LockRng);
      lpFoxLock         : Result := DBFHandler.UnLock(FOX_LockMax + 1 - FOX_LockRng, FOX_LockRng);
      lpClipperForFoxLob: Result := DBFHandler.UnLock(CLIPPER_FOR_FOX_LOB_LockMin + 1, CLIPPER_FOR_FOX_LOB_LockRng);
    end;
  end else
    Result := true;
        ...
        case qq.field_type of
          'M', 'B', 'G':
            begin
              ...
              Result.Position := 0;
              if Crypt.FActive then
                Crypt.Decrypt(LobHeader.BlockLength * dInt, TVKDBTStream(Result).Memory, LenLob);
              ...
              dbftDBFDataSet:
                begin
                  ...
                  if FLobVersion >= xBaseIV then begin
                    if Crypt.FActive then
                      Crypt.Decrypt(LobHeader.BlockLength * dInt, TMemoryStream(NestedStream).Memory, LenLob);
                  end else begin
                    if Crypt.FActive then
...
function TVKSmartDBF.SetNativeAutoInc(const FieldName: String;
  Value: DWORD): boolean;
var
  oFld: TField;
begin
...
function TVKSmartDBF.GetCurrentNativeAutoInc(const FieldName: String): DWORD;
var
  oFld: TField;
begin
  Result := DWORD(-1);
...
function TVKSmartDBF.GetNextNativeAutoInc(const FieldName: String): DWORD;
var
  oFld: TField;
begin
  Result := DWORD(-1);
...
begin
  ...
  for I := VarArrayLowBound(V, 1) to VarArrayHighBound(V, 1) do
    Result := Result + VarToStr(V[I]) + IfThen(I < VarArrayHighBound(V, 1), '|');
end;
...
begin
  FieldRec.FeildName := Value;
end;

function TVKDBFFieldDef.Get_field_type: char;
begin
  Result := FieldRec.field_type;
end;

procedure TVKDBFFieldDef.Set_field_type(const Value: char);
begin
...
function FIELD_REC.Get_field_type: char;
begin
  if not ( FFIELD_REC_VER in [xBaseV..xBaseVII] ) then
...
procedure FIELD_REC.Set_field_type(const Value: char);
begin
...
function FIELD_REC.Get_field_name: pChar;
begin
  if not ( FFIELD_REC_VER in [xBaseV..xBaseVII] ) then
...
function FIELD_REC.GetFeildName: String;
begin
  if not ( FFIELD_REC_VER in [xBaseV..xBaseVII] ) then
...
procedure FIELD_REC.SetFeildName(const Value: String);
var
  l, m: Integer;
begin
  l := Length(Value);
  ...
  Move(pChar(Value)^, FFIELD_REC_STRUCT.fldRecIII.field_name, m);
  m := l;
  if l > 31 then m := 31;
  FillChar(FFIELD_REC_STRUCT.fldRecV.field_name, 32, 0);
  Move(pChar(Value)^, FFIELD_REC_STRUCT.fldRecV.field_name, m);
end;
