...
var
  ...
  begin
    PDWord( pChar(Fpipe) + ( ( pipeIn mod FNULL ) * SizeOf(DWord) ) )^ := Value;
    Inc(pipeIn);
  end;

  function pipeGet: DWord;
  begin
    Result := PDWord( pChar(Fpipe) + ( ( pipeOut mod FNULL ) * SizeOf(DWord) ) )^;
    Inc(pipeOut);
  end;
...
begin
  ...
  if Assigned(GetHashCodeMethod) then begin
    ...
    FStandardHash := True;
    FGetHashCodeMethod := GetHashCode;
...
procedure TVKSorterAbstract.GetHashCode(Sender: TObject;
...
begin
  Result := PByte( pChar(FKeyPool) + ( ndx * ( ( SizeOf(DWord) shl 1 ) + FKeySize ) ) + ( SizeOf(DWord) shl 1 ) );
end;
*)

function TVKSorterAbstract.GetPSortItem(ndx: DWord): PSORT_ITEM;
begin
  Result := PSORT_ITEM(pChar(FKeyPool) + ( ndx * ( ( SizeOf(DWord) shl 1 ) + FKeySize ) ) );
end;

(*
function TVKSorterAbstract.GetRecNo(ndx: DWord): DWord;
begin
  Result := PDWord( pChar(FKeyPool) + ( ndx * ( ( SizeOf(DWord) shl 1 ) + FKeySize ) ) + SizeOf(DWord) )^;
end;
*)

function TVKSorterAbstract.GetSortArray(ndx: DWord): DWord;
begin
  Result := PDWord(pChar(FSortArray) + ( ndx * SizeOf(DWord) ))^;
end;
...
begin
  Move(Value^, PByte( pChar(FKeyPool) + ( ndx *  ( ( SizeOf(DWord) shl 1 ) + FKeySize ) ) + ( SizeOf(DWord) shl 1 ) )^, FKeySize);
end;
*)
...
begin
  PDWord( pChar(FKeyPool) + ( ndx * ( ( SizeOf(DWord) shl 1 ) + FKeySize ) + SizeOf(DWord) ) )^ := Value;
end;
*)
...
procedure TVKSorterAbstract.SetSortArray(ndx: DWord;
  const Value: DWord);
begin
  PDWord(pChar(FSortArray) + ( ndx * SizeOf(DWord) ))^ := Value;
end;
...
begin
  ...
  Move(PSORT_ITEM(@Value)^, PSORT_ITEM(pChar(FKeyPool) + ( ndx *  l ))^, l);
end;
...
begin
  Result := PDWord( pChar(Fcsrt) + ( ndx * SizeOf(DWord) ) )^;
end;

procedure TVKBinaryTreeSorter.SetCsrt(ndx: DWord; const Value: DWord);
begin
  PDWord( pChar(Fcsrt) + ( ndx * SizeOf(DWord) ) )^ := Value;
end;
...
begin
  ...
  Result := PSORT_ITEM(pChar(FKeyPool) + ( FCurrentItem * ( ( SizeOf(DWord) shl 1 ) + FKeySize ) ) );
end;
...
begin
  Result := PByte( pChar(Fcolor) + ( ndx * SizeOf(Byte) ) )^;
end;
...
begin
  PByte( pChar(Fcolor) + ( ndx * SizeOf(Byte) ) )^ := Value;
end;
...
var
  q: PShortInt;
begin
  q := PShortInt( pChar(Fbalance) + ( ndx * SizeOf(ShortInt) ) );
  Dec( q^ );
  result := q^;
end;
...
begin
  Result := PShortInt( pChar(Fbalance) + ( ndx * SizeOf(ShortInt) ) )^;
end;
...
begin
  PShortInt( pChar(Fbalance) + ( ndx * SizeOf(ShortInt) ) )^ := Value;
end;
...
begin
  ...
  //for i := 0 to Pred(FKeySize) do begin
    if CurrentNode = nil then begin
      ...
      //c := item.key[i] - CurrentNode.KeyChar;
...
begin
  ...
  //
  FTrackCurent := FKeySize;
  CurrentNode := FRoot.lson;
  for i := 0 to Pred(FKeySize) do begin
    FTrack[i] := CurrentNode;
    FTrack_Sort_Item.key[i] := FTrack[i].KeyChar;
    CurrentNode := CurrentNode.lson;
  end;
  FTrack[FKeySize] := CurrentNode;
...
begin
  Result := PPVKTrieNode( pChar(FHashTable) + SizeOf(PVKTrieNode) * Ind )^;
end;
...
begin
  Result := False;
  while FTrack[FTrackCurent].rson = nil do begin
  ...
  if FTrackCurent < FKeySize then
    FTrack_Sort_Item.key[FTrackCurent] := FTrack[FTrackCurent].KeyChar;
  FTrack_Sort_Item.RID := FTrack[FTrackCurent].RID;
  CurrentNode := FTrack[FTrackCurent].lson;
  for i := Succ(FTrackCurent) to FKeySize do begin
    FTrack[i] := CurrentNode;
    if i < FKeySize then
      FTrack_Sort_Item.key[i] := FTrack[i].KeyChar;
    FTrack_Sort_Item.RID := FTrack[i].RID;
    CurrentNode := CurrentNode.lson;
  end;
  FTrackCurent := FKeySize;
end;
...
begin
  PPVKTrieNode( pChar(FHashTable) + SizeOf(PVKTrieNode) * Ind )^ := Value;
end;
...
begin
  if FCurrent < FCount then
    Result := PVKTrieNode(pChar(FMemory) + SizeOf(TVKTrieNode) * FCurrent)
  else
    Result := nil;
  Inc(FCurrent);
end;
...
begin
  ...
  if Result = nil then begin
    FCurrentBlockSize := FCurrentBlockSize shr 1;
...
begin
  ...
  if not FInnerSorter.FirstSortItem then begin
    repeat
      ...
      sipnt := PSORT_ITEM(pChar(sipnt) + FSortItemSize);
      Inc(q);
...
begin
  Result := PSORT_ITEM(pChar(FBuff1) + FCurrentSortItemInBlock * FSortItemSize);
end;
  ...
  for JoinBlock := 1 to Pred(FBlockCount) do begin
    ...
    for CurrentBlock := 0 to Pred(JoinBlock) do begin
      ...
      while cnt1 < PDWord(FBuff1)^ do begin
        ...
        psi1 := PSORT_ITEM( pChar(FBuff1) + cnt1 * FSortItemSize );
        if cnt2 < PDWord(FBuff2)^ then begin
          psi2 := PSORT_ITEM( pChar(FBuff2) + cnt2 * FSortItemSize );
        ...
        sipnt := PSORT_ITEM( pChar(FBuff3) + PDWord(FBuff3)^ * FSortItemSize );
    ...
    while cnt2 < PDWord(FBuff2)^ do begin
      psi := PSORT_ITEM( pChar(FBuff2) + cnt2 * FSortItemSize );
      Inc(cnt2);
      sipnt := PSORT_ITEM( pChar(FBuff3) + PDWord(FBuff3)^ * FSortItemSize );
      Move((@psi.RID)^, (@sipnt.RID)^, FSortItemSize);
      Inc(PDWord(FBuff3)^);
...
var
  sipnt: PSORT_ITEM;
begin
  sipnt := PSORT_ITEM( pChar(Buffer) + SizeOf(DWord) + FSortItemSize * PMergeBlockHeader(Buffer).count );
  Move((@psi.RID)^, (@sipnt.RID)^, FSortItemSize);
...
begin
  if FEof then
    ...
    Result := PSORT_ITEM(pChar(FBuff) + SizeOf(DWord) + FCurrentSortItemInBlock * FSortItemSize);
end;
