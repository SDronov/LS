{
  ...
  $Revision: 270 $
  $Author: alexvolo $
  $Date: 2005-11-02 10:54:11 +0400 (РЎСЂ, 02 РЅРѕСЏ 2005) $
}
...
uses
  fdcUtils, fdcMessages, fdcCustomDataFrm, cxDBEdit;
...
procedure NavigateToFirst(ALayoutControl: TdxLayoutControl);
  function DoNavigate(AGroup: TdxLayoutGroup): Boolean;
  var
    I: Integer;
  begin
   Result := True;
   ...
   begin
      ...
      begin
        if DoNavigate(TdxLayoutGroup(AGroup.Items[I])) then Exit
      end
      else
        if (TdxLayoutItem(AGroup.Items[I]).Control is TWinControl ) and
          TWinControl(TdxLayoutItem(AGroup.Items[I]).Control).CanFocus then
         begin
           TWinControl(TdxLayoutItem(AGroup.Items[I]).Control).SetFocus;
           Exit;
         end;
   end;
   Result := False;
...
begin
  DoNavigate(ALayoutControl.Items);
end;
...
begin
  ...
  if lcGeneral.CanFocusEx then
  begin
    lcGeneral.SetFocus;
    NavigateToFirst(lcGeneral);
  end;
...
procedure TfdcCustomCardForm.FormKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if Key = #13 then
  begin
