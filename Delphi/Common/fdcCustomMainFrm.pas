{
  ...
  $Revision: 2808 $
  $Author: sbalezin $
  $Date: 2007-06-04 17:28:14 +0400 (РџРЅ, 04 РёСЋРЅ 2007) $
}
...
uses
  ...
  fdcCustomFrm, fdcCustomDBDataFrm;
...
begin
  ...
  try
    ...
    on Exception do begin // компонент не создался, но в список компонентов владельца попал
      AOwner.RemoveComponent(Result);
      raise;
    end;
  end;
...
begin
  ...
  try
   if Animation then SetAnimation(False);
    ...
    begin
      if (TApplication_(Application).FModalLevel = 0) and (AForm.FormStyle <> fsMDIChild) then
      begin
...
//        if AForm.WindowState = wsMinimized then
    ...
    if not Assigned(ADockSite) and (TApplication_(Application).FModalLevel > 0) then
    begin
       AForm.Position := poScreenCenter;
       AForm.ShowModal;
       PostMessage(AForm.Handle, CM_RELEASE, 0, 0);
    end
    else
    { показать форму }
     AForm.Show;

    if  (AForm is TfdcCustomDBDataForm) and AForm.Showing and
          not  TfdcCustomDBDataForm(AForm).dsData.Active then
    begin
      if AForm.HandleAllocated then
        PostMessage(AForm.Handle, WM_CLOSE, 0, 0);
      Exit;
    end;
