...
const
  maxFieldCountPUANS = 17;             // максимальное число полей в dbf из LS*.ANS
...
begin
  ...
  if parray[9] <> '' then
    dbf.FieldByName('SUMMA'     ).AsFloat := StrToFloat(parray[9]);       //Сумма платежа
  dbf.FieldByName('KBK'         ).AsString   := parray[10];               //Код бюджетной классификации
  dbf.FieldByName('PAYMENTNUM'  ).AsString   := parray[11];               //Номер документа основания для расходования денежных средств
  if parray[12] <> '' then
    dbf.FieldByName('PAYMENTDAT').AsDateTime := StrToDate(parray[12]);    //Дата платежного документа
  dbf.FieldByName('NAME_AGENT'  ).AsString   := parray[13];               //Наименование таможенного представителя
