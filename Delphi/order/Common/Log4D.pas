...
{$ENDIF}
  ...
  ISimplyLogger = interface
    ['{101F416D-7A1F-495D-B008-85B7B013DFF7}']
    ...
    procedure Fatal(const Message: TObject; const Err: Exception = nil);
      overload;
    procedure Info(const Message: string; const Err: Exception = nil); overload;
    procedure Info(const Message: TObject; const Err: Exception = nil);
      overload;
    procedure Warn(const Message: string; const Err: Exception = nil); overload;
  ...
  ISimplyLoggerFactory = interface(IUnknown)
    ['{0C820059-ED4F-4330-9AF7-1EF271AE773B}']
    function GetLogger(const Name: string): ISimplyLogger;
  end;

  ISimplyLoggerConfigurator = interface(IUnknown)
    ['{0C820059-ED4F-4330-9AF7-1EF271AE773B}']
    function Configure(const Props: TStrings): ISimplyLoggerFactory;
  end;
...
{ Logger factory --------------------------------------------------------------}
  ...
  ISimplyLogLoggerFactory = interface
    ['{15FA8FFF-EB7A-4CC1-8FE0-838DA6010CAA}']
    function GetLogger(const Name: string): ISimplyLogger;
  end;

  { Default implementation of a logger factory. }
  TLogDefaultLoggerFactory = class(TInterfacedObject, ILogLoggerFactory, ISimplyLogLoggerFactory)
  public
    function MakeNewLoggerInstance(const Name: string): TLogLogger;
    function GetLogger(const Name: string): ISimplyLogger;
  end;
...
{ Loggers ---------------------------------------------------------------------}
  ...
  TLogLogger = class(TLogOptionHandler, ILogOptionHandler, ISimplyLogger)
  private
    FAdditive: Boolean;
...
{ Configurators ---------------------------------------------------------------}
  ...
  public
    constructor Create;
    destructor Destroy; override;
    class procedure Configure(const Props: TStrings); overload; virtual;
    ...
    class procedure Configure(const Appender: ILogAppender = nil); overload;
  ...
  protected
    procedure ConfigureRootLogger(const Props: TStrings;
      const Hierarchy: TLogHierarchy);
    procedure ParseAdditivityForLogger(const Props: TStrings;
      const Logger: TLogLogger);
    function ParseAppender(const Props: TStrings;
      const AppenderName: string): ILogAppender;
    procedure ParseLoggersAndRenderers(const Props: TStrings;
      const Hierarchy: TLogHierarchy);
    procedure ParseLogger(const Props: TStrings; const Logger: TLogLogger;
      const Value: string);
  public
//    procedure Configure(const Props: TStrings); overload; override;
    class procedure Configure(const Props: TStrings); override;
    ...
    procedure DoConfigure(const Props: TStrings;
      const Hierarchy: TLogHierarchy); overload;
  end;
...
function TLogDefaultLoggerFactory.GetLogger(const Name: string): ISimplyLogger;
begin
  Result := TLogLogger.GetLogger(Name);
end;
...
begin
  FAppenders.Free;
  if assigned(DefaultHierarchy) and assigned(DefaultHierarchy.FLoggers) then
     DefaultHierarchy.FLoggers.Delete(DefaultHierarchy.FLoggers.IndexOfObject(Self));
...
function Relative2Absolute(const Path, FileName: string): string;
var
  CurDir: String;
begin
  if Path = '' then
    ...
    else Result := IncludeTrailingPathDelimiter(Path) + FileName;
end;
...
begin
  ...
  FAppend := Append;
  SetOption(FileNameOpt, Relative2Absolute(ExtractFileDir(FileName), ExtractFileName(FileName)));
end;
  ...
  try
    ...
    begin
      if FStream <> nil then
        FStream.Free;
//      FFileName := Value;
      FFileName := Relative2Absolute(ExtractFileDir(Value), ExtractFileName(Value));
      ...
      begin
        ...
        FStream := TFileStream.Create(CreateFile(PChar(FFileName),
        GENERIC_READ or GENERIC_WRITE,
         FILE_SHARE_READ, nil, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0));
//        FFileName, fmOpenWrite or fmShareDenyWrite);
      WriteHeader;
    end;
  finally
    LeaveCriticalSection(FCriticalAppender);
...
procedure SetSubProps(const Prefix: string; const Props: TStrings;
...
{ Read configuration options from properties.
  See DoConfigure for the expected format. }
class procedure TLogPropertyConfigurator.Configure(const Props: TStrings);
var
  Config: TLogPropertyConfigurator;
begin
  Config := TLogPropertyConfigurator.Create;
...
procedure TLogPropertyConfigurator.ConfigureRootLogger(const Props: TStrings;
  const Hierarchy: TLogHierarchy);
var
  Value: string;
begin
...
procedure TLogPropertyConfigurator.DoConfigure(const Props: TStrings;
  const Hierarchy: TLogHierarchy);
begin
...
procedure TLogPropertyConfigurator.ParseAdditivityForLogger(
  const Props: TStrings; const Logger: TLogLogger);
var
  Value: string;
begin
...
function TLogPropertyConfigurator.ParseAppender(const Props: TStrings;
  const AppenderName: string): ILogAppender;
var
...
procedure TLogPropertyConfigurator.ParseLoggersAndRenderers(
  const Props: TStrings; const Hierarchy: TLogHierarchy);
var
  Index: Integer;
  Key, Name: string;
  Logger: TLogLogger;
...
procedure TLogPropertyConfigurator.ParseLogger(const Props: TStrings;
...
class procedure TLogBasicConfigurator.Configure(const Props: TStrings);
begin
...
//  DefaultLoggerFactory._Release;
  DefaultLoggerFactory := nil;
//  DefaultHierarchy.Free;
  FreeAndNil(DefaultHierarchy);
  { Registration cleanup. }
//  AppenderNames.Free;
  FreeAndNil(AppenderNames);
//  AppenderClasses.Free;
  FreeAndNil(AppenderClasses);
//  ErrorHandlerNames.Free;
  FreeAndNil(ErrorHandlerNames);
//  ErrorHandlerClasses.Free;
  FreeAndNil(ErrorHandlerClasses);
//  FilterNames.Free;
  FreeAndNil(FilterNames);
//  FilterClasses.Free;
  FreeAndNil(FilterClasses);
//  LayoutNames.Free;
  FreeAndNil(LayoutNames);
//  LayoutClasses.Free;
  FreeAndNil(LayoutClasses);
//  LoggerFactoryNames.Free;
  FreeAndNil(LoggerFactoryNames);
//  LoggerFactoryClasses.Free;
  FreeAndNil(LoggerFactoryClasses);
//  RenderedNames.Free;
  FreeAndNil(RenderedNames);
//  RenderedClasses.Free;
  FreeAndNil(RenderedClasses);
//  RendererNames.Free;
  FreeAndNil(RendererNames);
//  RendererClasses.Free;
  FreeAndNil(RendererClasses);
  { NDC. }
  if NDC <> nil then
  begin
    while NDC.Count > 0 do
    begin
      I := NDC.Count - 1;
      NDC.Objects[I].Free;
      if I = NDC.Count - 1 then
        NDC.Delete(I);
    end;
//    NDC.Free;
    FreeAndNil(NDC);
