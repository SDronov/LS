unit CheckSvedReviseDeductAistLS;
...
type
  TCheckSvedReviseDeductAistLSForm = class(TfdcCustomDlgForm)
...
function  ExecReport(Session: TOracleSession): Boolean;
var
  F: TCheckSvedReviseDeductAistLSForm;
begin
  F := TCheckSvedReviseDeductAistLSForm.Create(nil);
  with F do
  try
    // --- init Sessions
...
procedure TCheckSvedReviseDeductAistLSForm.FormCreate(Sender: TObject);
begin
  inherited;
  dxMemData.Open;
  dxMemData.Insert;
...
procedure TCheckSvedReviseDeductAistLSForm.actOkExecute(Sender: TObject);
begin
