unit CheckSvedReviseChargeAistLS;
...
type
  TCheckSvedReviseChargeAistLSForm = class(TfdcCustomDlgForm)
...
function  ExecReport(Session: TOracleSession): Boolean;
var
  F: TCheckSvedReviseChargeAistLSForm;
begin
  F := TCheckSvedReviseChargeAistLSForm.Create(nil);
  with F do
  try
    // --- init Sessions
...
procedure TCheckSvedReviseChargeAistLSForm.FormCreate(Sender: TObject);
begin
  inherited;
  dxMemData.Open;
  dxMemData.Insert;
...
procedure TCheckSvedReviseChargeAistLSForm.actOkExecute(Sender: TObject);
begin
