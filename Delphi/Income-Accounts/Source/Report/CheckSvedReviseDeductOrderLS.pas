unit CheckSvedReviseDeductOrderLS;
...
type
  TCheckSvedReviseDeductOrderLSForm = class(TfdcCustomDlgForm)
...
function  ExecReport(Session: TOracleSession): Boolean;
var
  F: TCheckSvedReviseDeductOrderLSForm;
begin
  F := TCheckSvedReviseDeductOrderLSForm.Create(nil);
  with F do
  try
    // --- init Sessions
...
procedure TCheckSvedReviseDeductOrderLSForm.FormCreate(Sender: TObject);
begin
  inherited;
  dxMemData.Open;
  dxMemData.Insert;
...
procedure TCheckSvedReviseDeductOrderLSForm.actOkExecute(Sender: TObject);
begin
