unit CheckSvedReviseChargeOrderLS;
...
uses
  ...
  cxDBLookupEdit, cxDBLookupComboBox, fdcObjectLookupEdit, cxGroupBox,
  cxRadioGroup;

type
  TCheckSvedReviseChargeOrderLSForm = class(TfdcCustomDlgForm)
    ...
    dxMemDataPaytypeCode: TStringField;
    dxLayoutControl1Item1: TdxLayoutItem;
    rgReportType: TcxRadioGroup;
    dxMemDataReportType: TIntegerField;
...
function  ExecReport(Session: TOracleSession): Boolean;
var
  F: TCheckSvedReviseChargeOrderLSForm;
begin
  F := TCheckSvedReviseChargeOrderLSForm.Create(nil);
  with F do
  try
    // --- init Sessions
...
procedure TCheckSvedReviseChargeOrderLSForm.FormCreate(Sender: TObject);
begin
  inherited;
  dxMemData.Open;
  dxMemData.Insert;
...
procedure TCheckSvedReviseChargeOrderLSForm.actOkExecute(Sender: TObject);
begin
  ...
  dxMemDataReportType.Value := rgReportType.ItemIndex;
  //dsLong.SQL.Text := Format(BaseSQLLong, ['--', 'NULL']);
  dsLong.Params.ParseSQL(dsLong.SQL.Text, True);
  dsTotals1.Params.ParseSQL(dsTotals1.SQL.Text,True);
  //--
  // --- генерация отчета
  try
    //dsHeader.Open;
