inherited AdviceDecisionGridForm: TAdviceDecisionGridForm
  ...
  inherited BarManager: TdxBarManager
    ...
    object cmbDocType: TdxBarCombo
      ...
      Category = 0
      Hint = #1055#1086#1082#1072#1079#1072#1090#1100' '#1074#1099#1073#1088#1072#1085#1085#1099#1081' '#1090#1080#1087' '#1076#1086#1082#1091#1084#1077#1085#1090#1086#1074
      Visible = ivAlways
      Text = #1042#1089#1077
  ...
  inherited dsData: TfdcQuery
    SQL.Strings = (
      ...
      '              AND d.id IN ( SELECT ra.REL_OBJECT_ID'
      '                            FROM fdc_rel_advice_dec_lst ra'
      '                            WHERE ra.OBJECT_ID = :ID )'
      ''
      '      UNION ALL'
      ''
      ...
      ''
        ...
        '.state_id)AS StateName,'
      '             d.is_ts'
      '       FROM fdc_dec_payback_periodic_lst d'
      '       WHERE ( :doc_type in ( 0, 1 ) )'
      '              AND d.id IN ( SELECT rt.REL_OBJECT_ID'
      '                              FROM fdc_rel_advice_dec_lst rt'
      '                             WHERE rt.OBJECT_ID = :ID )'
      ...
      ''
        ...
        '.state_id)AS StateName,'
      '             d.is_ts'
      '       FROM fdc_dec_payback_pay_lst d'
      '      WHERE ( :doc_type in ( 0, 1 ) )'
      '             AND d.id IN ( SELECT rt.REL_OBJECT_ID'
      '                             FROM fdc_rel_advice_dec_lst rt'
      '                            WHERE rt.OBJECT_ID = :ID )'
      ...
      ''
        ...
        '.state_id ) AS StateName,'
      '             a.is_ts'
      '       FROM fdc_act_ded_overpay_lst a, fdc_subject_lst sb'
      '      WHERE ( :doc_type in ( 0, 2 ) )'
      '             AND sb.ID (+) = a.SUBJECT_ID'
      '             AND a.id IN ( SELECT v.OWNER_OBJECT_ID'
      '                             FROM fdc_value_lst v'
      '                            WHERE v.REF_OBJECT_ID = :ID )'
      '      UNION ALL'
        ...
        '.state_id)AS StateName,'
      '             d.is_ts'
      '       FROM FDC_DEC_PAYBACK_LEVY_lst d'
      '      WHERE ( :doc_type in ( 0, 1 ) )'
      '             AND d.id IN ( SELECT rt.REL_OBJECT_ID'
      '                             FROM fdc_rel_advice_dec_lst rt'
      '                            WHERE rt.OBJECT_ID = :ID )'
      '      UNION ALL'
        ...
        '.state_id)AS StateName,'
      '             d.is_ts'
      '       FROM FDC_DEC_PAYBACK_outback_lst d'
      '      WHERE ( :doc_type in ( 0, 1 ) )'
      '             AND d.id IN ( SELECT rt.REL_OBJECT_ID'
      '                             FROM fdc_rel_advice_dec_lst rt'
      '                            WHERE rt.OBJECT_ID = :ID )'
