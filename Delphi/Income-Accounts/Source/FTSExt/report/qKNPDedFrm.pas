...
uses
  ...
  fdcObjectLookupEdit, cxGraphics;

type
  TqKNPDedForm = class(TfdcCustomDlgForm)
    ...
    procedure dsParamKNP_IDValidate(Sender: TField);
...
var
  F: TqKNPDedForm;
begin
  F := TqKNPDedForm.Create(nil);
  try
    SetSession(F, Session);
    F.fdcObjectLookupEdit2.Properties.ListSource := F.srcPayerList;
//  F.dsPayerList.Open;
...
procedure TqKNPDedForm.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TqKNPDedForm.actOkExecute(Sender: TObject);
begin
//  inherited;
...
procedure TqKNPDedForm.dsMainBeforeQuery(Sender: TOracleDataSet);
begin
  inherited;
...
procedure TqKNPDedForm.xlReportDataSources1BeforeDataTransfer(
  DataSource: TxlDataSource);
begin
  inherited;
  if dsMain.Active then dsMain.Refresh
    else dsMain.Open;
  BuildDataSet(dsMain, dsData, 'DOC_NO;DOC_SUMMA;PO_SUMMA','KBK', 'SUMMA');
...
procedure TqKNPDedForm.BuildTemplate(DataSource: TxlDataSource; ADataSet: TDataSet);
const
  LeftRow = 7;
...
procedure TqKNPDedForm.dsPayerListBeforeQuery(Sender: TOracleDataSet);
begin
  inherited;
  Sender.SetVariable('ID', dsParamKNP_ID.AsVariant);
end;

procedure TqKNPDedForm.dsParamKNP_IDValidate(Sender: TField);
begin
  inherited;
//dsPayerList.Refresh;
//dsParamSUBJ_ID.Clear;
end;

procedure TqKNPDedForm.dsHeaderBeforeQuery(Sender: TOracleDataSet);
begin
  inherited;
...
procedure TqKNPDedForm.xlReportDataSources2BeforeDataTransfer(
  DataSource: TxlDataSource);
begin
  inherited;
//dsData.Filtered := False;
...
procedure TqKNPDedForm.dsDataFilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
  inherited;
		if DataSet.Filter = '0' then
				Accept := DataSet.FieldByName('DOC_NO').IsNull;
		if DataSet.Filter = '1' then
				Accept := not DataSet.FieldByName('DOC_NO').IsNull;
end;
