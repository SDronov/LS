{
  ...
  $Revision: 16476 $
  $Author: adavletyarov $
  $Date: 2013-04-03 12:29:53 +0400 (РЎСЂ, 03 Р°РїСЂ 2013) $
}
unit MainFrm;
...
type
  ...
  TMainForm = class(TfdcCustomOraMainForm, IfdcObjectServices)
    ...
    // обработчик TdxBarButton.OnClick для запуска отчетов
    // TdxBarButton.Description = 'Имя_Класса_Формы_Отчета;Имя_Файла_Шаблона'
...
const
  ...
  cptLevelRTU     = ' - РТУ';
  cptLevelFTS     = ' - ФТС';
  cptLevelFTSKNP  = ' - КНП уровня ФТС';
  cptLevelRTUKNP  = ' - КНП уровня РТУ';
    ...
    try
      ...
      begin
        E.Message :=
          ...
          '<<Приложение не может быть запущено. Обратитесь к администратору системы.>> ' +
          E.Message;
        ErrorDlg(E);
...
begin
  ...
  //-- скрыть - Настройки оперативного учета
...
begin
  ShowMessage('Отчет был исключен!');
  exit;
  MainData.CheckTask('Tsk_Report_Notice', True);
  qNoticeFrm.ExecReport(Session);
end;
...
begin
  ShowMessage('Отчет не реализован.');
  exit;
...
{  MessageDlg( 'Запрос в АПС "Задолженность" поставлен в очередь на обработку!'
             ,''
             ,''
             ,mtInformation
             ,[mbOK]
             );  }
//  MessageDlg('Запрос в АПС "Задолженность" поставлен в очередь на '+#13+#10+'обработку!', mtInformation, [mbOK], 0);
  ShowMessage('Запрос в АПС "Задолженность" поставлен в очередь на обработку!');
end;
...
begin
  case GetCustomsLevel of
    0 : vFile := 'Руководство пользователя ЛС-ФТС.htm';
    1 : vFile := 'Руководство пользователя ЛС-РТУ.htm';
    2 : vFile := 'Руководство пользователя ЛС-Т.htm';
    3,4 : vFile := 'Руководство пользователя ЛС-КНП.htm';
  end;
...
begin
  ...
  for i := 0 to FTaskListBarNavigator.Groups.Count - 1 do
    if FTaskListBarNavigator.Groups[i].Caption = 'Документы' then begin
...
begin
  ...
  if fShowVersion then
    begin
    //22.8.16
      fStAv := 'База Данных: ' + Session.LogonDatabase + MainData.GetHintRcd(Session.LogonUsername) + GetAppVer;
          ...
          if is_need_Reportbudgetgrid_present(( Data as TBuxReport ).ID) then
            ...
            begin
              ...
              try
                ...
                lReportBudgetGridForm.Caption := 'Бюджетные отчёты: ' + ( Data as TBuxReport ).Name;
                for li := 0 to Navigator.TreeView.Items.Count-1 do
                  begin
...
begin
  inherited;
  qMasterDetailByPeriodFrm.ExecReport('Отправленные_реестры.xls');
end;

procedure TMainForm.btnReportInPacksClick(Sender: TObject);
begin
  inherited;
  qMasterDetailByPeriodFrm.ExecReport('Принятые_реестры.xls');
end;

procedure TMainForm.btnCustomsChequeNewClick(Sender: TObject);
begin
  fdcReportDateForm_DateToUnit.ExecReport(Session, 'ТаможКарты');
end;
...
{$IFDEF _DEBUG_}
  Action:= caFree
{$ELSE}
  if ConfirmDlg( 'Вы уверены, что хотите выйти из программы "' + InternalName + '"?'
                ,'Подтвердите выход из программы' ) = mrYes then
    Action:= caFree
  else
    Action:= caNone;
{$ENDIF}
    ...
    if MainData.Session.Connected then
      while (MainData.Session.DBMS_Pipe.Receive_Message(UpperCase(MainData.Session.LogonUsername), 0) = 0) do
        begin
          ...
          sMesBf := sMesBf + '---------- Сообщение ----------' + #13#10 + sMes + #13#10;
        end;
...
begin
  ...
  if not(Assigned(vClass) and vClass.InheritsFrom(TXLSReportForm)) then
    raise Exception.CreateFmt('Неверное имя класса формы отчета [%s]!', [AFormClass]);
...
begin
 if not(Assigned(Sender) and (Sender is TdxBarButton)) then
   raise Exception.Create('Класс не назначен или назначен не верно!');
 vStr := TdxBarButton(Sender).Description;
 vPos := pos(';', vStr);
 if (vPos = 0) then
   raise Exception.Create('Не верно установлено свойство Description!');
 ShowXLSReportForm(
