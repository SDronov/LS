{
  ...
  $Revision: 1021 $
  $Author: charushin $
  $Date: 2006-02-07 17:27:51 +0400 (Р’С‚, 07 С„РµРІ 2006) $
}
...
uses
  fdcUtils, fdcCustomObjectGridFrm;
...
begin
  if AItem = OwnerForm then
    Result := OwnerForm.pcMain.TabCount
  else
...
begin
  ...
  if (not (Sender is TdxBarButton)) or
     (WarningDlg(
        ...
        'Если хотите выполнить данное действие нажмите "Да", иначе нажмите "Нет"'
      ) <> mrYes)
  then 
    Exit;
