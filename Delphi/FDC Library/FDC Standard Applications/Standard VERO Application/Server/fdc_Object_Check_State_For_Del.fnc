create or replace function fdc_Object_Check_State_For_Del
-- Возвращает допускает ли состояние объекта его удаление
-- 1 - допускает, 0 - нет
	(
...
begin
  ...
  -- Иначе смотрим допускает ли данное состояние удаление объекта   
     begin
       select	nvl(AllowDelete, 0)
	       into	Result
	       from	fdc_State
	      where	ID = pStateID;
     exception   
  ...
  -- Если нельзя удалять и нужно исключение, то поднимаем его  
  if (Result = 0) and (pDoRaise <> 0) then
	   fdc_util.abort('Состояние объекта не допускает его удаления.');
  end if;
  
  return Result;
end fdc_Object_Check_State_For_Del;
/
