create or replace procedure fdc_User_Group_Grant_Add
-- Выдать группе права на заданный объект
	(
	pUser_Group_ID	number,          -- Идентификатор группы, которой выдаются права
  ...
  pNoReduction  number := 0        -- не равно 0 - если группа уже имеет более высокие права на объект,
...
begin
  ...
  -- Есть ли уже какие-либо права этой группе на заданный объект?
  begin
    select AccessLevel	
    into	pAcc
  ...
  exception
  	...
  	when others then fdc_util.abort('Ошибка определения прав группы на объект', 1);
  end;
      
  -- pAcc > 0 - уже есть в fdc_User_Group_Grant запись о правах, 
  -- pAcc = 0 - нет 
  if pAcc > 0 then
    -- Если группа уже имеет такие права, то выходим    
    if (pAccessLevel = pAcc) then 
       return;
    end if;
    
    -- Если pNoReduction выставлен и группа уже имеет более высщкие права,
    ...
    pEventID := fdc_Event_Add('EventAccessUpd',
          ...
          ' для группы ' || fdc_Object_Get_Name(pUser_Group_ID) ||
          ' изменен с "' || fdc_Access_Level_GetName(pAcc) ||
          '" на "' || fdc_Access_Level_GetName(pAccessLevel), pObject_ID);   
  else
    -- Добавление уровня доступа
  	insert into fdc_User_Group_Grant(User_Group_ID, Object_ID, AccessLevel)
  	values(pUser_Group_ID, pObject_ID, pAccessLevel);
