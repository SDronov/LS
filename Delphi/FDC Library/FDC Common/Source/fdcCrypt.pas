{
  Библиотека шифрования данных.

  Copyright © Fors Development Center.
  All rights reserved.

  $Workfile: fdcCrypt.pas $
  $Revision: 3 $
  $Author: alexvolo $
  $Date: 2005-07-18 16:23:50 +0400 (РџРЅ, 18 РёСЋР» 2005) $

  Links:
    PC1 CIPHER with 128-bit keys (16 symbols max) by
    Alexander PUKALL 1991.
    This stream cipher algorithm can be used to encrypt
    texts or binary files. Provides very high security.
}
unit fdcCrypt;
...
function EncryptEx(Key, Source: string): string;
function DecryptEx(Key, Source: string): string;

implementation

const
  SDefaultKey = 'vghf*7uh9k!jp-09';

var
  ax, bx, cx, dx, si, tmp, x1a2, res, i, inter, cfc, cfd, compte, j, l : Word;
...
procedure InternalEncrypt(ThisCle, Buffer: PChar; BufferLength: Integer);
...
Procedure InternalDecrypt(ThisCle, Buffer: PChar; BufferLength: Integer);
var
  Rep: Char;
  c, d, e: Byte;
begin
...
function Encrypt(Source: string): string;
begin
  Result := EncryptEx(SDefaultKey, Source);
end;

function Decrypt(Source: string): string;
begin
  Result := DecryptEx(SDefaultKey, Source);
end;
...
begin
  ...
  InternalEncrypt(PChar(Key), PChar(Source), Length(Source));
  Result := Cry;
end;
...
begin
  ...
  InternalDecrypt(PChar(Key), PChar(Source), Length(Source));
  Result := Cry;
end;
