...
begin
  ...
  try
    ...
    TLogLogger.GetRootLogger.Info('    Пользователь ЕЛС: ' + fDBUserNameFTS );
    TLogLogger.GetRootLogger.Info('    БД ЕЛС:           ' + fDBServerFTS );
  ...
  if not AQuery.Active then
    if WaitForSingleObject( fHSemQueue, INFINITE ) = WAIT_OBJECT_0 then
      begin
        ...
        TLogLogger.GetRootLogger.Info( Format( 'В очередь добавлен запрос %s от %s. Запросов в очереди: %d', [ AQuery.Key, AQuery.ConnectionName, fQueueList.Count ] ) );
        if not ReleaseSemaphore( fHSemQueue, 1, nil ) then
          TLogLogger.GetRootLogger.Error( Format( 'Ошибка при переводе семафора очереди в сигнальное состояние: %d', [ GetLastError ] ) );
      end
  else
...
begin
  if WaitForSingleObject( fHSemQueue, INFINITE ) = WAIT_OBJECT_0 then
    begin
      ...
      TLogLogger.GetRootLogger.Info( Format( 'Исключён из очереди запрос %s от %s. Запросов в очереди: %d', [ AQuery.Key, AQuery.ConnectionName, fQueueList.Count ] ) );
      if not ReleaseSemaphore( fHSemQueue, 1, nil ) then
        TLogLogger.GetRootLogger.Error( Format( 'Ошибка при переводе семафора очереди в сигнальное состояние: %d', [ GetLastError ] ) );
    end;
end;
