...
const
  ...
  maxFieldCountPUANS = 16;             // максимальное число полей в dbf из LS*.ANS
...
{}
  ...
  TCetTableDescPUANS = record
    ...
    Fields     : TCetFieldsPUANS;         // двумерный массив в котором храняться описания полей
  end;
...
function Create_DBF_PUANS(ExportDir : string; tblDesc : TCetTableDescPUANS) : string;   //Создание dbf файла
procedure Filling_DBF_PUANS(val: string; parray: array of string);  //заполнение dbf файла
...
begin
  ...
  if parray[6] <> '' then
    dbf.FieldByName('REPORT_DAT').AsDateTime := StrToDate(parray[6]);     //Дата подтверждения уплаты таможенных пошлин, налогов
  dbf.FieldByName('NUMDOC'      ).AsString   := parray[7];                //Номер платежного документа
  if parray[8] <> '' then
    dbf.FieldByName('SUMMA'     ).AsFloat := StrToFloat(parray[8]);       //Сумма платежа
  dbf.FieldByName('KBK'         ).AsString   := parray[9];                //Код бюджетной классификации
  dbf.FieldByName('PAYMENTNUM'  ).AsString   := parray[10];               //Номер документа основания для расходования денежных средств
  if parray[12] <> '' then
    dbf.FieldByName('PAYMENTDAT').AsDateTime := StrToDate(parray[11]);    //Дата платежного документа
  dbf.FieldByName('NAME_AGENT'  ).AsString   := parray[12];               //Наименование таможенного представителя
  dbf.FieldByName('NUMPOST'     ).AsString   := parray[13];               //Наименование загружаемой транзакции
  dbf.FieldByName('CODEERR'     ).AsString   := parray[14];               //Код ошибки
  dbf.FieldByName('TEXTERR'     ).AsString   := parray[15];               //Описание ошибки
