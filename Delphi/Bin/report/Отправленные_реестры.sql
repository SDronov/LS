...
[dsMaster]
-- Отправленные реестры, поле ID - обязательно, для Master-Detail
SELECT distinct dp.OBJECT_TYPE_ID AS id, dp.TypeName
FROM fdc_doc_pack_transfer_out_lst tro, fdc_doc_pack_lst dp, fdc_object_relation_lst r
WHERE r.Object_ID = tro.id 
  AND dp.id = r.Rel_Object_ID
  AND trunc(tro.SEND_DATE) BETWEEN :DATE_FROM AND :DATE_TO
...
SELECT ROWNUM, cc.*
 FROM (
    SELECT dp.DOC_NUMBER
          ...
          ,tro.customs_in_code AS Customs_Code
          ,trunc(tro.SEND_DATE) AS SEND_DATE
